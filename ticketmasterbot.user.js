...
function SkipPopup() {
  var popupPresent = getElementByXpath(
...
function ProcessFilterPanel(filterBar) {
  ...
  waitForElement(".offer-card", function () {
    ...
    var getTicketsElement = ClickElement(
      '//button[@id = "offer-card-buy-button"]'
    );
    ...
    waitForElement(".button-aux, .modal-dialog__button", function () {
      var sectionChangeBuyButton = getElementByXpath(
        '//button[@class = "button-aux modal-dialog__button"]'
      );
...
function ChangeTicketQuantity() {
  var rightPanelCurrentTicketCountElement = getElementByXpath(
    '//div[@class = "qty-picker__number qty-picker__number--lg"]'
  );
  ...
  if (ticketQuantityDifference > 0) {
    var ticketIncrementElement = ClickElement(
      '//button[@class = "qty-picker__button qty-picker__button--increment qty-picker__button--lg"]'
    );
    ...
    var ticketDecrementElement = ClickElement(
      '//button[@class = "qty-picker__button qty-picker__button--decrement qty-picker__button--lg"]'
    );
    for (var i = 0; i < ticketQuantityDifference; i++) {
      try {
